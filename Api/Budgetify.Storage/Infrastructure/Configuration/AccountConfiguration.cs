...
internal class AccountConfiguration : EntityTypeConfigurationBase<Account>, IEntityTypeConfiguration<Account>
{
    public void Configure(EntityTypeBuilder<Account> builder)
    {
        ...
        builder.Property(x => x.CurrencyFk).HasColumnName("CurrencyFk").HasColumnType("int").IsRequired();
        builder.Property(x => x.Type).HasColumnName("Type").HasColumnType("nvarchar(MAX)");

        builder.HasOne(x => x.User).WithMany(x => x.Accounts).HasForeignKey(x => x.UserFk);
        builder.HasOne(x => x.Currency).WithMany(x => x.Accounts).HasForeignKey(x => x.CurrencyFk);
